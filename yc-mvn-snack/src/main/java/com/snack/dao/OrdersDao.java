...
public class OrdersDao {
	...
	//查询待发货的订单详情
		...
		public void updateStatus(String ono) {
			String sql="UPDATE orderinfo o1,orderiteminfo o2 SET o1.`status`=2,o2.`status`=2 WHERE o1.ono=o2.ono AND o1.ono=?";
			new DBHelper().update(sql, ono);
		}
		
		/**
		 * 待评论当订单的所有商品的评价状态都为4时 才执行更新订单状态为4
		 */
		public void update(String ono) {
			String sql="UPDATE orderinfo o1,orderiteminfo o2 SET o1.`status`=4 WHERE o1.ono=o2.ono AND o2.`status`=4 AND o1.ono=?";
			new DBHelper().update(sql, ono);
		}
		//更新商品状态为已收货的状态变为3
		public int updateStatus3(String ono) {
			String sql = "UPDATE orderinfo o1,orderiteminfo o2 SET o1.`status`=3,o2.`status`=3 WHERE o1.ono=o2.ono AND o1.ono=?";
